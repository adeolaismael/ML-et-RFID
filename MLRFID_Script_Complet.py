...
# La fonction calcule la valeur maximale de Rssi pour chaque combinaison d''Epc', 'refListId', 'subslice_id' et 'loc' à partir du DataFrame tags. Les valeurs sont stockées dans un DataFrame nommé ana.
    ana = tags.groupby(['Epc','refListId','slice_id','loc'])['Rssi'].max()\
...
# Rajoute un _in en suffix pour les élements fusionnés venant de anna_in et réciproquement avec _out
    ana = pd.merge(ana_in, ana_out, on=['Epc', 'refListId'], suffixes=['_IN', '_OUT'], how = 'inner') \
            .sort_values(['Epc'])
...
def compare_resultats(resultats, noms_methodes):
    
    # On convertie la liste de décimal resultat en pourcentage
    resultats = [resultats*100 for resultats in resultats]

    # Créer les positions des barres
    positions = np.arange(len(noms_methodes))

    # Créer l'histogramme avec les résultats
    plt.bar(positions, resultats, align='center', width=0.5, edgecolor='black')
    ...
    # Définir les positions des marqueurs sur l'axe des abscisses et leurs étiquettes correspondantes
    plt.xticks(positions, noms_methodes)
    
    # On limite la graduation des y entre 50 et 100
    plt.ylim(bottom=50, top=100)

    # Formater les étiquettes des graduations de l'axe y en ajoutant le symbole de pourcentage (%)
    plt.gca().set_yticklabels(['{:.0f}%'.format(label) for label in plt.gca().get_yticks()])
    
    # Ajouter les valeurs sur chaque barre
    for i, resultat in enumerate(resultats):
        plt.text(i, resultat, '{:.1f}%'.format(resultat), ha='center', va='bottom')
